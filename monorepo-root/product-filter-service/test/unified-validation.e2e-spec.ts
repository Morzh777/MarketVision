...
import { UnifiedHybridValidator } from '../src/validators/unified-hybrid.validator';
...
describe('Unified Validation System (E2E)', () => {
  ...
  describe('Single Product Validation', () => {
    it('should validate RTX 5080 videocard correctly', () => {
      const result = validator.validate('RTX 5080', 'MSI RTX 5080 Gaming X Trio 16GB', 'videocards');
      expect(result.isValid).toBe(true);
      expect(result.reason).toContain('code-validated');
      expect(result.confidence).toBeGreaterThan(0.5);
    });

    it('should reject accessory products', () => {
      const result = validator.validate('RTX 5080', 'Кабель для RTX 5080', 'videocards');
      expect(result.isValid).toBe(false);
      expect(result.reason).toBe('accessory-words');
    });

    it('should validate processor correctly', () => {
      const result = validator.validate('7800X3D', 'AMD Ryzen 7 7800X3D', 'processors');
      expect(result.isValid).toBe(true);
      expect(result.confidence).toBeGreaterThan(0.5);
    });
  });
  ...
  describe('Configuration-based Validation', () => {
    it('should handle unknown category gracefully', () => {
      const result = validator.validate('test', 'Test Product', 'unknown_category');
      expect(result.isValid).toBe(false);
      expect(result.reason).toBe('не содержит запрос');
    });

    it('should validate different categories with different rules', () => {
      // Тест для процессоров
      const processorResult = validator.validate('i7-13700K', 'Intel Core i7-13700K', 'processors');
      expect(processorResult.isValid).toBe(true);

      // Тест для PlayStation
      const playstationResult = validator.validate('PS5', 'Sony PlayStation 5 Console', 'playstation');
  ...
  describe('Edge Cases', () => {
    it('should handle empty product name', () => {
      const result = validator.validate('RTX 5080', '', 'videocards');
      expect(result.isValid).toBe(false);
    });

    it('should handle very short product names', () => {
      const result = validator.validate('RTX 5080', 'RTX', 'videocards');
      expect(result.isValid).toBe(false);
      expect(result.reason).toBe('слишком короткое название');
    });

    it('should handle products without query match', () => {
      const result = validator.validate('RTX 5080', 'PlayStation 5 Console', 'videocards');
      expect(result.isValid).toBe(false);
      expect(result.reason).toBe('не содержит запрос');
    });
  });
