...
/**
 * Нормализует query для поиска - приводит к единому формату
 */
...
export function createSearchVariants(query: string): string[] {
  ...
  if (transliteratedFromRussian !== query) {
    variants.push(normalizeQuery(transliteratedFromRussian));
  }
  ...
  if (transliteratedFromEnglish !== query) {
    variants.push(normalizeQuery(transliteratedFromEnglish));
  }
