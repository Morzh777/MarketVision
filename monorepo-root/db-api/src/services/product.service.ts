...
export class ProductService {
    ...
    try {
      await this.prisma.marketStats.create({
        data: {
          product_id:
            'product_id' in stats
              ? (stats as { product_id: string }).product_id
              : stats.productId,
          query: stats.query,
          ...
          total_count:
            'total_count' in stats
              ? (stats as { total_count: number }).total_count
              : stats.totalCount,
   ...
   */
  async getMarketStats(query: string): Promise<MarketStats | null> {
  ...
  > {
    console.log('[ProductService] getPopularQueries called with:', {
      telegram_id,
      category,
      limit,
      offset,
    ...
    if (category && category !== 'all') {
      // Получаем ozon_id и wb_id по ключу категории
      const categoryRecord = await this.prisma.category.findUnique({
        where: { key: category },
        select: { display: true, ozon_id: true, wb_id: true },
      });

      if (categoryRecord) {
        // Ищем товары по ozon_id или wb_id
        whereCondition.OR = [
          ...(categoryRecord.ozon_id
            ? [{ category: categoryRecord.ozon_id }]
            : []),
          ...(categoryRecord.wb_id ? [{ category: categoryRecord.wb_id }] : []),
        ];
        console.log(
          `[ProductService] Filtering by category: ${category} -> ozon_id: ${categoryRecord.ozon_id}, wb_id: ${categoryRecord.wb_id}`,
        );
      } else {
        console.log(`[ProductService] Category not found: ${category}`);
        // Если категория не найдена, возвращаем пустой результат
        return [];
      }
    }

    // Получаем уникальные запросы с пагинацией + 1 для проверки есть ли еще данные
    const queries = (await this.prisma.product.findMany({
      ...
      distinct: ['query'],
      skip: offset,
      take: limit + 1, // Берем на 1 больше для проверки
    })) as Array<{ query: string; category: string }>;
    ...
    console.log('[ProductService] Where condition:', whereCondition);

    // Если получили меньше чем limit + 1, значит это последняя страница
    const isLastPage = queries.length <= limit;
    const actualQueries = isLastPage ? queries : queries.slice(0, limit);

    console.log(
      '[ProductService] Actual queries to process:',
    ...
    const result = await Promise.all(
      actualQueries.map(async ({ query }) => {
        ...
        if (latestProduct) {
          console.log(
            '[ProductService] latestProduct.category:',
          ...
          console.log('[ProductService] categoryKey:', categoryKey);
        }
    ...
    // Если это последняя страница и после фильтрации у нас меньше элементов чем ожидалось,
    // возвращаем пустой массив чтобы фронт понял что данных больше нет
    if (isLastPage && filteredResult.length < actualQueries.length) {
      console.log(
        '[ProductService] Last page detected, returning empty array to stop pagination',
      );
      return [];
    }

    console.log(
      '[ProductService] Returning result:',
