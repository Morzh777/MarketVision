...
class TestOzonRawProductService:
    ...
    def service(self, mock_parser_service: Mock) -> OzonRawProductService:
        ...
        with patch(
            "infrastructure.grpc.ozon_grpc_service.OzonParserService",
            return_value=mock_parser_service,
        ):
    ...
    async def test_get_raw_products_with_platform_id(
        ...
        # Act
        response = await service.GetRawProducts(
            sample_request_with_platform, mock_context
        )

        # Assert
        assert len(response.products) == 2
        mock_parser_service.parse_products.assert_called_once_with(
            "Nintendo Switch", "igrovye-pristavki", "nintendo-switch"
        )

    @pytest.mark.asyncio
    async def test_get_raw_products_empty_query_returns_invalid_argument(
        self, service: OzonRawProductService, mock_context: Mock
    ) -> None:
    ...
    async def test_get_raw_products_parser_unavailable_returns_unavailable(
        ...
        mock_context.set_details.assert_called_with(
    ...
    async def test_get_raw_products_parser_exception_returns_internal_error(
        ...
        # Arrange
        mock_parser_service.is_available = AsyncMock(return_value=True)
        mock_parser_service.parse_products = AsyncMock(
        ...
        mock_context.set_details.assert_called_with(
            "Internal parser error: Network error"
        )
    ...
    async def test_get_raw_products_product_conversion_error_skips_product(
        ...
        products_with_error = [
            ...
            Product(
                id="2", name="Invalid Product", price=float("inf"), category="test"
            ),  # Вызовет ошибку
        ...
        assert (
            len(response.products) >= 2
        )  # Может быть 2 или 3, зависит от обработки inf
        assert response.total_count >= 2
