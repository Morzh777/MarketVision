...
class TestOzonParserService:
    ...
    def service(self, mock_parser: Mock) -> OzonParserService:
        ...
        with patch('infrastructure.services.ozon_parser_service.OzonParser', return_value=mock_parser):
    ...
    async def test_parse_products_success(
        ...
        mock_parser.get_products.assert_called_once_with("RTX 4090", "videokarty-15721", None)
    ...
    async def test_parse_products_with_platform_id(
        ...
        # Act
        result = await service.parse_products("RTX 4090", "videokarty-15721", "nintendo-switch")
        
        # Assert
        assert len(result) == 2
        mock_parser.get_products.assert_called_once_with("RTX 4090", "videokarty-15721", "nintendo-switch")
    ...
    @pytest.mark.asyncio
    async def test_is_available_initial_state(
        self, 
        service: OzonParserService
    ) -> None:
